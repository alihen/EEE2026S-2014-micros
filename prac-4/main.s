...
copy_to_RAM:
        ...
        LDR R0, =0x20000000             @Set Pointer at Start of Ram
        LDR R1, =start_of_data          @Set Source Pointer at Start of Data
        LDR R2, =end_of_data            @Set Pointer for end of data.    
        SUBS R2, R2, R1                   
        ADDS R2, R2, R0                 @Calculate the last word used in ram - initially done by just comparing the end pointer, but it broke my byte add later on.

copy_loop:
        LDR R3,[R1]
        STR R3,[R0]
        ADDS R1, R1, #4                 @Move both pointers
        ADDS R0, R0, #4
        CMP R0, R2                       @Check if we've reached the end of the loop.
        BNE copy_loop
        
copy_to_RAM_complete:
        @ Critical! Once the program hits this label, the automarker will modify the contents of RAM.
        @ iterate through each individual *byte* in the array, add one to it and write it back.

        LDR R0, =0x20000000              @Reset RAM pointer to Start of RAM

 byte_add:                              @Adapted from pg 114 byte copy loop in textbook
        LDRB R1,[R0]
        ADDS R1, R1, #1                 @increment 1 byte only.
        STRB R1,[R0]
        ADDS R0, R0, #1
        CMP R0, R2                       @check if we've reached the end of the array elements in RAM.
        BNE byte_add

increment_of_bytes_complete:            
        @ Critical! Here, the automarker will verify that each byte has been incremented.
        ...
        LDR R0, =0x20000000              @Set Pointer at Start of Ram 
        LDRB R4, [R0]                    @Set 1st element as Max value to begin the loop

get_max_unsigned_loop:
        LDRB R1, [R0]
        CMP R1, R4                       @Compare array element in RAM to R4 (Stored element), branch out if array element is higher
        BHI set_max_unsigned
        ADDS R0, R0, #1
        CMP R0, R2
        BNE get_max_unsigned_loop
        
        B reset_ram
        
set_max_unsigned:                       @Set new max element - Saved in R4
        MOVS R4, R1
        B get_max_unsigned_loop

reset_ram:
        LDR R0, =0x20000000
        LDRB R5, [R0]                   @Set 1st element as Min value to begin the loop

get_min_unsigned_loop:
        LDRB R1, [R0]
        CMP R1, R5
        BCC set_min_unsigned            @If (R0 < R1) then branch BCC label / BLO label - page 109
        ADDS R0, #1
        CMP R0, R2
        BNE get_min_unsigned_loop

        B display_conditional_on_pushbuttons       
           
set_min_unsigned:
        MOVS R5, R1
        B get_min_unsigned_loop
